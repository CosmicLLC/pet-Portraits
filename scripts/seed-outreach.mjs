// One-time seed: import the 9 pet blogger drafts into OutreachContact.
// Idempotent — checks handle before creating.
//
...
Erinc
pawmasterpiece.com`,
    notes: "Editorial-first ask, with paid-placement option named so they don't have to fish for it.",
  },
  {
...
Erinc
pawmasterpiece.com`,
    notes: "Co-marketing angle as instructed. The sitter-story observation is real positioning.",
  },
  {
    name: "Dogster team",
    channel: "blogger",
...
Erinc
pawmasterpiece.com`,
    notes: "Straight paid-editorial inquiry — short and transactional because the contact form is the gate, not a relationship.",
  },
  {
...
Erinc
pawmasterpiece.com`,
    notes: "Pure FYI + media kit ask. They don't take editorial.",
  },
  {
...
Erinc
pawmasterpiece.com`,
    notes: "Treats the media kit as homework done. Memorial portrait mention is honest. Email is in the media kit PDF.",
  },
  {
    name: "BlogPaws team",
    channel: "blogger",
...
Erinc
pawmasterpiece.com`,
    notes: "Frames as a relationship, not a transaction.",
  },
];
...
async function main() {
  ...
  const existing = await prisma.outreachContact.findMany({ select: { handle: true } });
  const have = new Set(existing.map((e) => e.handle));
  const toCreate = SEED.filter((s) => !have.has(s.handle));
  if (toCreate.length === 0) {
    console.log("All 9 contacts already seeded.");
    await prisma.$disconnect();
    return;
  }
  for (const s of toCreate) {
    await prisma.outreachContact.create({ data: s });
  }
  console.log(`✓ Seeded ${toCreate.length} contacts`);
  const all = await prisma.outreachContact.findMany({
    select: { handle: true, priority: true, status: true, email: true },
    orderBy: { priority: "asc" },
  });
  for (const c of all) {
    console.log(`  P${c.priority} ${c.handle.padEnd(32)} ${c.email || "(no email — uses URL)"}`);
  }
  await prisma.$disconnect();
}
